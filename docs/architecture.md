...
```
スキル（config/functions/）        ← 最小機能単位。エージェントが自作。
  └─ スキル（config/skills/） ← スキルの組み合わせ手順書。
...
```json
{
  ...
  "cron": "0 23 * * *",
  "prompt": "スキル（/app/config/skills/market_news_recipe.md）に従って米国株式市場ニュースを投稿してください。",
  "channelId": "チャンネルID",
  "enabled": true
}
```
