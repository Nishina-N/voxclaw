...
function buildSystemInstruction(
    base: string,
    examples: Array<{ transcription: string; intent: string }>,
): string {
    const lines = examples.map(e => `「${e.transcription}」→「${e.intent}」`).join('\n');
    return `${base}

# ユーザーの過去の発話例（参考）
以下の例を参考に、このユーザーの発話パターンや音声認識の誤変換を考慮して意図を推定してください。
${lines}`;
}
...
export async function createLiveSession(
    ...
    languageCode?: string,
    intentExamples: Array<{ transcription: string; intent: string }> = [],
): Promise<GeminiLiveSession> {
    const ai = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY! });

    const baseInstruction = mode === 'faithful'
        ? SYSTEM_INSTRUCTION_FAITHFUL
        : SYSTEM_INSTRUCTION_STANDARD;

    const systemInstruction = intentExamples.length > 0
        ? buildSystemInstruction(baseInstruction, intentExamples)
        : baseInstruction;
