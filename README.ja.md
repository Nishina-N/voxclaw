...
ソースコードは [Polyform Noncommercial License 1.0](LICENSE) で公開しています。

**個人・研究・教育目的は無償で利用可能。** 商用利用には別途ライセンスが必要です。ご相談ください。

> ⚠️ **特許出願中（2026年）：**「音声入力 → 意図推定 → ユーザー確認・編集 → 実行」のパイプラインは特許出願中です。本ソフトウェアの利用は商用目的の特許ライセンスを付与するものではありません。
