...
const SYSTEM_INSTRUCTION_STANDARD = `あなたは音声アシスタントです。
...
【重要】ユーザーが話している間、または report_intent を呼んだ後は、自分から音声応答しないでください。
「【返答】:」で始まるメッセージを受け取ったときだけ、その内容を日本語で自然に読み上げてください。`;

// Faithful mode: transcription is shown as-is, no interpretation.
const SYSTEM_INSTRUCTION_FAITHFUL = `あなたは音声アシスタントです。
...
export async function createLiveSession(
    ...
    const session = await ai.live.connect({
        ...
        callbacks: {
            ...
            onmessage: (msg: any) => {
                // Capture audio output chunks from the model (TTS playback)
    ...
    return {
        ...
        speakText(text: string) {
            try {
                ...
                console.warn('[gemini] speakText error:', e?.message);
            }
        },
        close() {
